...
func (vq *VoteQuery) QueryQuestion() *QuestionQuery {
	query := (&QuestionClient{config: vq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := vq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (vq *VoteQuery) QueryAnswer() *AnswerQuery {
	query := (&AnswerClient{config: vq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := vq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (vq *VoteQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Vote, error) {
	...
	if query := vq.withQuestion; query != nil {
		if err := vq.loadQuestion(ctx, query, nodes,
			func(n *Vote) { n.Edges.Question = []*Question{} },
			func(n *Vote, e *Question) { n.Edges.Question = append(n.Edges.Question, e) }); err != nil {
			return nil, err
		}
	}
	if query := vq.withAnswer; query != nil {
		if err := vq.loadAnswer(ctx, query, nodes,
			func(n *Vote) { n.Edges.Answer = []*Answer{} },
			func(n *Vote, e *Answer) { n.Edges.Answer = append(n.Edges.Answer, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (vq *VoteQuery) loadQuestion(ctx context.Context, query *QuestionQuery, nodes []*Vote, init func(*Vote), assign func(*Vote, *Question)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(vote.QuestionTable)
		s.Join(joinT).On(s.C(question.FieldID), joinT.C(vote.QuestionPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(vote.QuestionPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(vote.QuestionPrimaryKey[1]))
...
func (vq *VoteQuery) loadAnswer(ctx context.Context, query *AnswerQuery, nodes []*Vote, init func(*Vote), assign func(*Vote, *Answer)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*Vote)
	nids := make(map[int]map[*Vote]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(vote.AnswerTable)
		s.Join(joinT).On(s.C(answer.FieldID), joinT.C(vote.AnswerPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(vote.AnswerPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(vote.AnswerPrimaryKey[1]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
