...
func (aq *AnswerQuery) QueryComments() *CommentQuery {
...
func (aq *AnswerQuery) QueryVotes() *VoteQuery {
	query := (&VoteClient{config: aq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := aq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (aq *AnswerQuery) WithComments(opts ...func(*CommentQuery)) *AnswerQuery {
	query := (&CommentClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (aq *AnswerQuery) WithVotes(opts ...func(*VoteQuery)) *AnswerQuery {
	query := (&VoteClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (aq *AnswerQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Answer, error) {
	...
	if aq.withAuthor != nil || aq.withQuestion != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := aq.withComments; query != nil {
		if err := aq.loadComments(ctx, query, nodes,
			func(n *Answer) { n.Edges.Comments = []*Comment{} },
			func(n *Answer, e *Comment) { n.Edges.Comments = append(n.Edges.Comments, e) }); err != nil {
			return nil, err
		}
	}
	if query := aq.withVotes; query != nil {
		if err := aq.loadVotes(ctx, query, nodes,
			func(n *Answer) { n.Edges.Votes = []*Vote{} },
			func(n *Answer, e *Vote) { n.Edges.Votes = append(n.Edges.Votes, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (aq *AnswerQuery) loadComments(ctx context.Context, query *CommentQuery, nodes []*Answer, init func(*Answer), assign func(*Answer, *Comment)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*Answer)
	nids := make(map[int]map[*Answer]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(answer.CommentsTable)
		s.Join(joinT).On(s.C(comment.FieldID), joinT.C(answer.CommentsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(answer.CommentsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(answer.CommentsPrimaryKey[0]))
...
func (aq *AnswerQuery) loadVotes(ctx context.Context, query *VoteQuery, nodes []*Answer, init func(*Answer), assign func(*Answer, *Vote)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*Answer)
	nids := make(map[int]map[*Answer]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(answer.VotesTable)
		s.Join(joinT).On(s.C(vote.FieldID), joinT.C(answer.VotesPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(answer.VotesPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(answer.VotesPrimaryKey[0]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
