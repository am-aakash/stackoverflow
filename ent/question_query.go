...
func (qq *QuestionQuery) QueryComments() *CommentQuery {
	query := (&CommentClient{config: qq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := qq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (qq *QuestionQuery) QueryVotes() *VoteQuery {
	query := (&VoteClient{config: qq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := qq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (qq *QuestionQuery) Clone() *QuestionQuery {
	...
	return &QuestionQuery{
		...
		withComments: qq.withComments.Clone(),
		withTags:     qq.withTags.Clone(),
		withVotes:    qq.withVotes.Clone(),
...
func (qq *QuestionQuery) WithComments(opts ...func(*CommentQuery)) *QuestionQuery {
	query := (&CommentClient{config: qq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	qq.withComments = query
	return qq
}
...
func (qq *QuestionQuery) WithVotes(opts ...func(*VoteQuery)) *QuestionQuery {
	query := (&VoteClient{config: qq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (qq *QuestionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Question, error) {
	...
	if query := qq.withComments; query != nil {
		if err := qq.loadComments(ctx, query, nodes,
			func(n *Question) { n.Edges.Comments = []*Comment{} },
			func(n *Question, e *Comment) { n.Edges.Comments = append(n.Edges.Comments, e) }); err != nil {
			return nil, err
		}
	}
	...
	if query := qq.withVotes; query != nil {
		if err := qq.loadVotes(ctx, query, nodes,
			func(n *Question) { n.Edges.Votes = []*Vote{} },
			func(n *Question, e *Vote) { n.Edges.Votes = append(n.Edges.Votes, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (qq *QuestionQuery) loadComments(ctx context.Context, query *CommentQuery, nodes []*Question, init func(*Question), assign func(*Question, *Comment)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(question.CommentsTable)
		s.Join(joinT).On(s.C(comment.FieldID), joinT.C(question.CommentsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(question.CommentsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(question.CommentsPrimaryKey[0]))
...
func (qq *QuestionQuery) loadVotes(ctx context.Context, query *VoteQuery, nodes []*Question, init func(*Question), assign func(*Question, *Vote)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*Question)
	nids := make(map[int]map[*Question]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(question.VotesTable)
		s.Join(joinT).On(s.C(vote.FieldID), joinT.C(question.VotesPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(question.VotesPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(question.VotesPrimaryKey[0]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
