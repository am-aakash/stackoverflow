...
func (cq *CommentQuery) QueryQuestion() *QuestionQuery {
...
func (cq *CommentQuery) QueryAnswer() *AnswerQuery {
...
func (cq *CommentQuery) WithQuestion(opts ...func(*QuestionQuery)) *CommentQuery {
	query := (&QuestionClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (cq *CommentQuery) WithAnswer(opts ...func(*AnswerQuery)) *CommentQuery {
	query := (&AnswerClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (cq *CommentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Comment, error) {
	...
	if query := cq.withQuestion; query != nil {
		if err := cq.loadQuestion(ctx, query, nodes,
			func(n *Comment) { n.Edges.Question = []*Question{} },
			func(n *Comment, e *Question) { n.Edges.Question = append(n.Edges.Question, e) }); err != nil {
			return nil, err
		}
	}
	if query := cq.withAnswer; query != nil {
		if err := cq.loadAnswer(ctx, query, nodes,
			func(n *Comment) { n.Edges.Answer = []*Answer{} },
			func(n *Comment, e *Answer) { n.Edges.Answer = append(n.Edges.Answer, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CommentQuery) loadQuestion(ctx context.Context, query *QuestionQuery, nodes []*Comment, init func(*Comment), assign func(*Comment, *Question)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(comment.QuestionTable)
		s.Join(joinT).On(s.C(question.FieldID), joinT.C(comment.QuestionPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(comment.QuestionPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(comment.QuestionPrimaryKey[1]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "question" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (cq *CommentQuery) loadAnswer(ctx context.Context, query *AnswerQuery, nodes []*Comment, init func(*Comment), assign func(*Comment, *Answer)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*Comment)
	nids := make(map[int]map[*Comment]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(comment.AnswerTable)
		s.Join(joinT).On(s.C(answer.FieldID), joinT.C(comment.AnswerPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(comment.AnswerPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(comment.AnswerPrimaryKey[1]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
